...
class AlgorithmsManager: ObservableObject, AlgorithmsManaging {
    ...
    private func data(for step: any SolveStep) -> AlgorithmGroupData {
        guard let url = Bundle.main.url(forResource: step.file, withExtension: "json"),
              let data = try? Data(contentsOf: url),
              let decoded = try? JSONDecoder().decode(AlgorithmGroupData.self, from: data)
        else { fatalError("Couldn't decode data for step '\(step.title)'") }
    ...
    func testAlgorithms(
        ...
        let baseArray = includeLearned ? learnedAndLearningAlgorithms : learningAlgorithms
...
extension Array {
    func grouping<Key: Hashable>(by grouper: (Element) -> Key) -> [Key: [Element]] {
